 ...
 * 
 * 为解决事务的一致性问题而产生的隔离机制:<br>
 * Read uncommited:未提交读,任何问题都解决不了,但是效率最高<br>
 * Read commited:已提交读,解决赃读,oracle使用这种默认方式<br>
 ...
 * {@link TransactionDefinition#PROPAGATION_REQUIRED}:<br>
 * 默认传播行为,若A有事务,则使用A的事务,若没有则新建事务<br>
 * {@link TransactionDefinition#PROPAGATION_SUPPORTS}:<br>
 * 如果A有事务,则使用A的事务,如果A当前没有事务,就不使用事务<br>
 * {@link TransactionDefinition#PROPAGATION_MANDATORY}:<br>
 * 如果A有事务,则使用A的事务,如果A当前没有事务,就抛出异常<br>
 * {@link TransactionDefinition#PROPAGATION_REQUIRES_NEW}:<br>
 * 新建事务,如果A当前存在事务,把当前事务挂起;如果A中有事务,则将A的事务挂起,新建一个事务,且只作用于B方法
 * {@link TransactionDefinition#PROPAGATION_NOT_SUPPORTED}:<br>
 * 以非事务方式执行操作,如果当前存在事务,就把当前事务挂起;如果A中有事务,则将A的事务挂起,不使用事务
 * {@link TransactionDefinition#PROPAGATION_NEVER}:<br>
 * 以非事务方式执行,如果当前存在事务,则抛出异常;如果A中有事务,直接报异常
 * {@link TransactionDefinition#PROPAGATION_NESTED}:<br>
