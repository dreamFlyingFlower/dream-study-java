...
public class MyJdbcClient {
	...
	public void test1() {
		// 使用占位符取单个值
		TestData testData = jdbcClient.sql("select * from my_data where id = ?")
				// 参数值,需要和sql中的位置对应
				.params("id的值")
				// 结果类型
		...
		// 使用占位符取多个值
		List<TestData> testDatas = jdbcClient.sql("select * from my_data where username = ?")
				.params("username的值")
				.query(TestData.class)
				.list();
		...
		List<TestData> testDatas2 = jdbcClient.sql("select * from my_data where username = :username")
				.params(params1)
				.query(TestData.class)
				.list();
		System.out.println(testDatas2);

		// 可只从查询结果中取某些值构建结果,适用于联表查询,结果不从一个表中取值
		jdbcClient.sql("select * from my_data")
				.query((rs, rowNum) -> new TestData(rs.getLong("id"), rs.getString("username"),
						rs.getBigDecimal("salary")))
				.list();

		// 查询记录数
		jdbcClient.sql("select count(*) from my_data where username = ?")
				.params("username的值")
				.query(Integer.class)
				.single();
		...
		// 按参数名新增
		jdbcClient.sql("insert into my_data values(:id,:username) ")
				.param("id", "id的值")
		...
		jdbcClient.sql("insert into my_data values(:id,:username,:salary) ")
				.paramSource(new TestData(1L, "username", BigDecimal.ZERO))
				.update();
