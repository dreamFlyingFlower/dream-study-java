 ...
 * 
 * <pre>
 * HashMap通过key的hashCode经过扰动函数处理过后得到hash值,然后通过 (n - 1) & hash 判断当前元素存放的位置(n 指的是数组的长度),
 * 如果当前位置存在元素的话,就判断该元素与要存入的元素的 hash 值以及 key 是否相同,
 * 如果相同,直接覆盖,不相同就通过拉链法解决冲突,然后存入链表中
 * 如果头节点是Node,则尾随它的就是一个普通的链表;如果头节点是TreeNode,它的后面就是一颗红黑树,TreeNode是Node的子类
 * 扰动函数:指 HashMap 的 hash 方法,使用 hash 方法是为了防止一些实现比较差的 hashCode() 方法,可以减少碰撞
 * 拉链法:将链表和数组相结合,也就是数组中每一格就是一个链表.若遇到哈希冲突,则将冲突的值加到链表中即可
 * </pre>
 * 
 * 使用红黑树的好处:一方面降低了Hash冲突,另一方面也提升了并发度
 * 
 * <pre>
* 使用红黑树,当一个槽里有很多元素时,其查询和更新速度会比链表快很多,Hash冲突的问题由此得到较好的解决
* 加锁的粒度并非整个ConcurrentHashMap,而是对每个头节点分别加锁,即并发度,就是Node数组的长度,初始长度为16
* 并发扩容,这是难度最大的.当一个线程要扩容Node数组的时候,其他线程还要读写,因此处理过程很复杂
 * </pre>
 * 
