...
import com.wy.utils.RequestUtils;
...
/**
 * ProcessFlow的抽象模板类,对其他子类通用方法的一些抽取,已经模板抽象方法的一些定义
 * 
 * @author 飞花梦影
 * @date 2021-01-07 15:01:24
...
public abstract class AbstractProcessFlow implements ProcessFlow {
	...
	/**
	 * UA user agent 意思是用户代理.用户代理是一种对数据打包,创造分组头,以及编址,传递消息的部件.
	...
	public String getHttpClientRequestData(String url, Map<String, String> parameterMap, List<CrawlerCookie> cookieList,
		...
		if (Objects.isNull(jsonDate)) {
			log.error("HttpClient 请求数据异常,url:{},parameter:{},cookies:{},proxy:{}", url, parameterMap,
					JSON.toJSONString(cookieList), proxy);
		}
		return jsonDate;
	}
