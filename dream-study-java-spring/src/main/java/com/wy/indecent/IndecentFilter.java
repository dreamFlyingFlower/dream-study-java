...
public class IndecentFilter {
	...
	// 单例
	private static IndecentFilter instance = null;

	/**
     * 构造函数，初始化敏感词库
    */
    private IndecentFilter() throws IOException {
        sensitiveWordMap = new IndecentWordInit().initKeyWord();
    }

	/**
	 * 获取单例
	 */
	public static IndecentFilter getInstance() throws IOException {
		if (null == instance) {
			instance = new IndecentFilter();
		}
		return instance;
	}
	...
	public int CheckSensitiveWord(String txt, int beginIndex, int matchType) {
		...
		for (int i = beginIndex; i < txt.length(); i++) {
			...
			if (nowMap != null) {
				...
				if ("1".equals(nowMap.get("isEnd"))) {
					...
					if (IndecentFilter.minMatchType == matchType) {
						break;
					}
				}
			}
		...
		if (IndecentFilter.maxMatchType == matchType || IndecentFilter.minMatchType == matchType) {
