 ...
 */
@Configuration
public class SelfSpringEventListener {
	...
	public void test() {
		// 第一种发布监听的方式
		SpringApplication application = new SpringApplication(SelfSpringEventListener.class);
		application.addListeners(new SelfApplicationListener());
		...
		ConfigurableApplicationContext applicationContext =
				SpringApplication.run(SelfSpringEventListener.class, new String[0]);
		// 添加监听器
		applicationContext.addApplicationListener(new SelfApplicationListener());
		// 启动完成之后发布监听
		applicationContext.publishEvent(new SelfSpringEvent("test"));
		configurableApplicationContext.close();
		// 使用以下方式同上
		// 该类实现了{ApplicationEventPublisher},对所有的事件进行发布和监听
		AnnotationConfigApplicationContext context = new AnnotationConfigApplicationContext();
		// 添加监听器
		context.addApplicationListener(new SelfApplicationListener());
		// 上下文启动
		context.refresh();
