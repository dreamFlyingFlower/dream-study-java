 ...
 *
 * @author 飞花梦影
 * @date 2024-04-01 14:01:37
 * @git {@link https://github.com/dreamFlyingFlower}
 */
public class EasyExcelMergeStrategy implements RowWriteHandler {
	...
	public EasyExcelMergeStrategy(Class<?> elementType) {
		...
		if (lastRow.getCell(primaryKeyIndex).getStringCellValue()
			...
			for (Integer mergeIndex : mergeColumnIndexList) {
				CellRangeAddress cellRangeAddress =
						new CellRangeAddress(row.getRowNum() - 1, row.getRowNum(), mergeIndex, mergeIndex);
	...
	private void initPrimaryIndexAndMergeIndex(WriteSheetHolder writeSheetHolder) {
		...
		for (Field field : fields) {
			...
			// 获取自定义注解，用于合并单元格
			EasyExcelMerge excelMerge = field.getAnnotation(EasyExcelMerge.class);
			// 判断是否需要合并
			if (null == excelMerge) {
				continue;
			}
