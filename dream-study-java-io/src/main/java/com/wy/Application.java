 ...
 * 
 * NIO:同步非阻塞流,适用于高并发,短连接的架构,如聊天室.主要类,,
 * NIO在read/write的时候是同步的,要等待操作完成,但和操作系统交互是异步的.
 * NIO在while读取客户端的流时不会阻塞,BIO会阻塞,多线程情况下,NIO比BIO性能更出色,消耗资源更小,且更稳定
 * 在读取流中的数据时,将不会一直等待,而是通过轮询来判断其他线程是否也需要进行读写操作.
 * 
