...
/**
 * 根据hibernate的注解检查实例对象是否正常 需配合hibernate,要在实体类上添加相应的注解org.hibernate.validator.constraints.*
 * 
 * @author paradiseWy
 */
public class ValidationUtils {
	...
	public static <T> void validateProperty(T obj, String propertyName) {
		Set<ConstraintViolation<T>> sets = validator.validateProperty(obj, propertyName, Default.class);
		if (sets != null && !sets.isEmpty()) {
			StringBuilder msg = new StringBuilder();
			for (ConstraintViolation<T> constraint : sets) {
				msg.append(constraint.getMessage()).append("<br>");
