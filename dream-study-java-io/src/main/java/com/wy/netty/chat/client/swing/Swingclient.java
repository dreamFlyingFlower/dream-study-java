package com.wy.netty.chat.client.swing;
...
import com.wy.netty.chat.client.Client;
import com.wy.netty.chat.client.swing.constant.ButtonCommand;
	...
	public void actionPerformed(ActionEvent event) {
		...
		case ButtonCommand.REGISTER:
			try {
				...
				// 构建请求
				Request request =
						Request.valueOf(ModuleId.PLAYER, PlayerCmd.REGISTER_AND_LOGIN, registerRequest.getBytes());
				client.sendRequest(request);
			} catch (Exception e) {
				tips.setText("无法连接服务器");
			}
		...
		case ButtonCommand.SEND:
			try {
				// 判断是广播还是私聊
				if (!StringUtils.hasText(targetPlayer.getText()) && StringUtils.hasText(message.getText())) {
					...
					// 构建请求
					Request request =
							Request.valueOf(ModuleId.CHAT, ChatCmd.PRIVATE_CHAT, privateChatRequest.getBytes());
					client.sendRequest(request);
				}
			} catch (Exception e) {
				tips.setText("无法连接服务器");
